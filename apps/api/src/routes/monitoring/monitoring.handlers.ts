...
import type { CheckSiteRoute, GetSitePingsRoute, GetAllWebsitesRoute } from './monitoring.routes'
...
export const checkSite: AppRouteHandler<CheckSiteRoute> = async (c) => {
  try {
    ...
    const pingResult = await db.insert(pings).values({
      ...
      error: result.error,
      // location and regionCode can be added later when implementing geo-location
